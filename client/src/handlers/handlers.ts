...
export async function getUsername(walletAddress: string) {
  ...
  return await fetch(
    "https://localhost:8080/getuser/" + walletAddress,
    requestOptions
  )
...
export async function createUsername(username: string, walletAddress: string) {
  ...
  return await fetch("https://localhost:8080/createuser", requestOptions)
    .then((response) => response.json())
    .catch((error) => console.log("error", error));
}
...
export async function changeUsername(username: string, walletAddress: string) {
  ...
  return await fetch("https://localhost:8080/updateuser", requestOptions)
    .then((response) => response.json())
